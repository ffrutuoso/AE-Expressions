...
(function rd_Statesman(thisObj)
{
    ...
    rd_StatesmanData.scriptTitle = rd_StatesmanData.scriptName + " v3.0";
    ...
    rd_StatesmanData.strMinAE100 = {en: "This script requires Adobe After Effects CS5 or later."};
            ...
            if (stateOpts !== null)
            {
                ...
                if (stateSwitchQual)
                    switch (layer.quality)
                    {
                        ...
                        case LayerQuality.BEST:
                            ...
                            {
                                if (layer.samplingQuality === LayerSamplingQuality.BILINEAR)
                                    layerInfo += "2";
                                else if (layer.samplingQuality === LayerSamplingQuality.BICUBIC)
                                    layerInfo += "3";
                                else
                                    layerInfo += " ";
                            }
                ...
                if (stateSwitch3D)
                {
                    ...
                    if (layer.threeDLayer)
                        layerInfo += (layer.threeDPerChar) ? "2" : "1";
                    else
                        layerInfo += "0";
                }
                else
...
//		alert("set state "+stateNum);
        ...
        for (var i=0; i<maxLayers; i++)
        {
            ...
            try
            {
                ...
                else if ((layerInfo[7] === "3") && (parseFloat(app.version) >= 12.0))
                {
                    layer.quality = LayerQuality.BEST;
            ...
            try
            {
                ...
                else if (layerInfo[12] === "1")
                {
                    layer.threeDLayer = true;
                    if (layer instanceof TextLayer)
                        layer.threeDPerChar = false;
                }
                else if ((layerInfo[12] === "2") && (layer instanceof TextLayer)) // for per-char 3D text
                {
                    layer.threeDLayer = true;
    ...
    if (parseFloat(app.version) < 10.0)
        alert(rd_Statesman_localize(rd_StatesmanData.strMinAE100), rd_StatesmanData.scriptName);
    else
    {
